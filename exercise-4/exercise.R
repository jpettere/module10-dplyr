...
histogram.flights <- hist(flights$time_gained)

# On average, did flights gain or lose time?
# Note: use the `na.rm = TRUE` argument to remove NA values from your aggregation
mean.gain <- mean(flights$time_gained, na.rm = TRUE)
print(mean.gain)

# Create a data.frame of flights headed to seatac ('SEA'), only including the
# origin, destination, and the "gain" column you just created
flight.to.sea = flights %>%
  filter(dest == "SEA") %>%
  select(origin, dest, time_gained)

# On average, did flights to seatac gain or loose time?
mean <- mean(flight.to.sea$time_gained, na.rm = TRUE)
print(mean)
