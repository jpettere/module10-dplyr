...
vehicles.1997 <- mutate(vehicles.1997, average = ((hwy + cty) / 2))

# Filter the whole vehicles data set for 2-Wheel Drive vehicles that get more than 20 miles/gallon in the city
# Save this new data frame in a variable.
vehicles.two.wheel <- filter(vehicles, drive == '2-Wheel Drive', cty > 20)

# Of those vehicles, what is the vehicle ID of the vehicle with the worst hwy mpg?
# Hint: filter for the worst vecle, then select its ID.
filtered <- filter(vehicles.two.wheel, hwy == min(hwy))
id <- select(filtered, id)
...
MakeYearFilter <- function(year, make) {
  filtered <- filter(vehicles, year == year, make == make)  
